...
local Window = Library.CreateLib("MVS GHOST - VENEZUELA", "Midnight")

local Players = game:GetService("Players")
local User = Players.LocalPlayer

-- FUNCIÓN PARA BUSCAR AL ENEMIGO
...
_G.SilentAim = true
CSec:NewToggle("Perfect Silent Aim", "No falla nunca", function(state) _G.SilentAim = state end)
...
-- 2. TAB DE GHOST (INVISIBILIDAD)
local GhostTab = Window:NewTab("Ghost")
local GSec = GhostTab:NewSection("Modo Fantasma")

GSec:NewButton("Hacerse Invisible", "Los demás no te verán", function()
    ...
    game.StarterGui:SetCore("SendNotification", {Title = "Ghost Mode", Text = "Ya eres un fantasma...", Duration = 3})
end)

-- 3. TAB DE VISUALES (ESP)
...
-- 4. TAB DE EVENTO (PASCUA - AUTO FARM)
